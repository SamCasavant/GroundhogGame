// The move_actor function handles final adjustments to the path in the event of
// a potential collision with another entity. It acts on entities with a
// position, destination, and path (vector of positions). Positions are shifted
// off of the path when a step is taken. When the path is completed or needs to
// be regenerated, the component is removed. crate::world::plan_path() produces
// a new path for entities that have none.
//
// TODO:
// Allow actors to rejoin path rather than starting over in the event of a
// correction. Create a system for near objectives to save aStar effort.
// Find a better way to store currently occupied positions? (Using hashmap for
// now) Improve GroundType integration with tile map.
...
pub fn move_actor(
    ...
    ) in &mut query.iter_mut()
    {
        if path.0.is_empty() {
            ...
            // the destination.
            if tilemap.map.contains_key(&next_step)
                && tilemap.map[&next_step].occupied
            {
                commands.entity(entity).remove::<world::Path>();
                ...
                for step in &temp_steps {
                    if !tilemap.map.contains_key(&step.0)
                        || !tilemap.map[&step.0].occupied
                    {
                        let new_distance =
                ...
                if next_step != *position {
                    ...
                    // If we've moved off of our path, we'll need to get a
                    // new one on the next loop.
                    // TODO: This should be changed
                    // to only if we
                    // cannot immediately rejoin path.
                }
            } else {
                path.0.remove(0);
            }
            ...
            let mut tile =
                tilemap.map.entry(*position).or_insert(world::Tile {
